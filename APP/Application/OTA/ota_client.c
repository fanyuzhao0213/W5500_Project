...
static uint8_t g_boot_confirmed = 0;                 // 本次启动已确认 (防止重复调用)
static uint8_t g_ota_success_notified = 0;           // 本次启动已上报 OTA 成功 (防止重复通知)

/* ==================== 内部辅助函数 ==================== */

/**
 * @brief 判断当前固件运行在哪个分区 (APP_A 或 APP_B)
 *
 * 实现原理:
 *   取本函数自己的 PC 地址 (即函数在 Flash 中的位置), 判断它落在
 *   App-A (0x0800C000~0x08084000) 还是 App-B (0x08084000~0x080FC000) 分区。
 *
 * 关键修复:
 *   旧实现使用 static 变量的地址 (在 RAM 0x20000000+) 来判断分区,
 *   但 RAM 地址永远大于 APP_B_ADDR+APP_B_SIZE, 导致判断永远返回 APP_A。
 *   这会让 App-B 运行时也把 app_b_valid 错误地写为 0, app_a_valid 写为 1,
 *   下次 bootloader 启动时会切换到无效的 App-A 而卡死。
 *
 * @return APP_A (0) 或 APP_B (1)
 */
static uint32_t get_running_app(void)
{
    /* 使用本函数的代码地址 (在 Flash, 跟随固件所在分区) */
    uint32_t fn_addr = (uint32_t)(void *)&get_running_app;

    if (fn_addr >= APP_A_ADDR && fn_addr < (APP_A_ADDR + APP_A_SIZE)) {
        return APP_A;
    }
    if (fn_addr >= APP_B_ADDR && fn_addr < (APP_B_ADDR + APP_B_SIZE)) {
        return APP_B;
    }

    /* 兜底: 异常地址 (理论上不应发生), 保守返回 APP_A */
    LOGW("get_running_app: unexpected fn_addr=0x%08lX, defaulting to APP_A",
...
void ota_client_init(void)
{
    ...
    // 从 OTA 参数区读取当前版本号
    // 判断自己运行在哪个分区 - 使用代码地址 (在 Flash 中) 而非 static 变量地址 (在 RAM)
    uint32_t running_app = get_running_app();
...
int ota_download_firmware(const char *url, uint32_t size, uint32_t expected_crc)
{
    ...
    // 注: 不需要从 OTA 任务调用 mqtt_client_loop_nonblocking()
    //     1. 该函数本应由 MQTT 任务独占调用, 跨任务调用会与 MQTT 任务竞争访问内部状态
    //     2. MQTT keepalive 是 60s, 完整擦除最多 ~8s, MQTT 任务自身的循环已能维护连接
    //     3. 旧代码在这里循环 50 次 (无 sleep), 会霸占 CPU 影响 MQTT 任务调度
...
//        flash_dump(target_addr, g_ota_status.total_size);
    ...
    /* 关键: 触发软重启, 让 bootloader 根据 ota_params.active_app 跳到新分区
     *
     * 延迟 2 秒的目的:
     *   1. 给 MQTT 任务把最后一条 stage=5 status 消息发出去
     *   2. 给 server 收到"升级成功"状态, 显示正常
     *   3. 避免重启太快导致 server 看不到 stage=5
     */
    osDelay(2000);

    LOGI("OTA: Triggering NVIC_SystemReset() now...");
    ota_report_status();  // 最后再报一次, 抓 log buffer

    /* 禁用所有中断, 防止重启前还有任务在跑 */
    __disable_irq();

    /* 触发 Cortex-M4 系统复位 (走 bootloader, 由 bootloader 根据 ota_params.active_app 跳转) */
    NVIC_SystemReset();
...
/**
 * @brief 上报 OTA 状态变更事件到服务器 (新固件启动后调用)
 *
 * 用途: 在新固件联网后, 通知服务器 OTA 升级结果
 *
 * 触发条件:
 *   - ota_flag == OTA_FLAG_SUCCESS  -> 发送 "ota_success" 事件
 *   - ota_flag == OTA_FLAG_ROLLBACK -> 发送 "ota_rollback" 事件
 *
 * 注意:
 *   - 同一启动周期内只上报一次 (g_ota_success_notified 去重)
 *   - 旧固件分区在 SUCCESS 之后才能被作为下次 OTA 的 target
 *   - 失败/回滚时旧分区仍然有效, 不影响下次 OTA
 *
 * @return OTA_ERR_OK 上报成功或无需上报, OTA_ERR_FLASH_WRITE 上报失败
 */
int ota_notify_boot_success(void)
{
    ...
    /* 去重: 同一启动周期只发一次 */
    if (g_ota_success_notified) {
        return OTA_ERR_OK;
    }

    /* 读取 OTA 参数 */
    err = ota_params_read(&params);
    if (err == OTA_PARAMS_FLASH_ERROR || err == OTA_PARAMS_ERROR || err == OTA_PARAMS_CHECKSUM_ERROR) {
        LOGW("ota_notify_boot_success: failed to read params, err=%d", err);
        return OTA_ERR_FLASH_WRITE;
    }

    /* 根据 ota_flag 选择事件类型 */
    if (params.ota_flag == OTA_FLAG_SUCCESS) {
        event_str = "ota_success";
    } else if (params.ota_flag == OTA_FLAG_ROLLBACK) {
        event_str = "ota_rollback";
    } else {
        /* NORMAL 或 PENDING 状态无需通知 */
        LOGI("ota_notify_boot_success: ota_flag=%lu, no notification needed",
             (unsigned long)params.ota_flag);
        return OTA_ERR_INVALID_PARAM;
    }

    LOGI("ota_notify_boot_success: event=%s, notifying server...", event_str);

    /* 构建通知消息 */
    memset(&pub_msg, 0, sizeof(pub_msg));

    /* 获取当前运行分区和版本号 (使用代码地址判断, 不依赖 RAM 地址) */
    uint32_t running_app = get_running_app();
    uint32_t current_ver = (running_app == APP_A) ? params.app_a_version : params.app_b_version;

    len = snprintf(pub_msg.payload, sizeof(pub_msg.payload),
        "{\"event\":\"%s\",\"app\":%lu,\"version\":%lu,\"boot_count\":%lu,\"active\":%lu}",
        event_str,
        (unsigned long)running_app,
        (unsigned long)current_ver,
        (unsigned long)params.boot_count,
        (unsigned long)params.active_app);

    snprintf(pub_msg.topic, sizeof(pub_msg.topic), "%s", OTA_TOPIC_NOTIFY);
    pub_msg.len = strlen(pub_msg.payload);
    pub_msg.qos = QOS1;  /* 通知用 QOS1, 确保服务器收到 */

    /* 放入发送队列 */
    if (mqtt_tx_queue_put(&pub_msg, 0) == 0) {
        LOGI("ota_notify_boot_success: notification queued to %s", OTA_TOPIC_NOTIFY);
        LOGI("ota_notify_boot_success: payload = %s", pub_msg.payload);
        g_ota_success_notified = 1;
        return OTA_ERR_OK;
    } else {
        LOGE("ota_notify_boot_success: failed to queue notification");
        return OTA_ERR_FLASH_WRITE;
    }
}

/**
 * @brief 新固件启动确认 (三阶段 A/B 升级第 2 步 - 简化为单次确认)
 *
 * 流程 (简化设计):
 *   1. 读取 OTA 参数
 *   2. 若 ota_flag == PENDING:
 *      - 视为升级已成功 (新固件已启动 + 联网成功 = 稳定)
 *      - 主动写 ota_flag = SUCCESS 到 Flash
 *      - 调用 ota_notify_boot_success() 通知服务器
 *   3. 若 ota_flag == SUCCESS:
 *      - 调用 ota_notify_boot_success() 通知服务器 (app 之前可能没成功写 SUCCESS)
 *   4. 若 ota_flag == NORMAL/ROLLBACK: 跳过
 *
 * 设计决策 (按用户最新要求):
 *   - "只要第一次成功 就可以认为是成功了 不需要5次"
 *   - 联网成功 = 升级成功
 *   - boot_count 仍由 bootloader 维护, 作为"app 失效"时的安全网
 *     (如果 app 永远没机会写 SUCCESS, bootloader 在 max_boot_count 次后会兜底写)
 *
 * 调用时机: 在新固件成功连接 MQTT 后调用一次
 * (本函数有去重, 同一启动周期内重复调用只生效一次)
 *
 * @return OTA_ERR_OK 成功, OTA_ERR_INVALID_PARAM 无需确认
 */
int ota_confirm_boot_success(void)
{
    ota_params_t params;
    ota_params_err_t err;
    uint32_t running_app;
    ...
    err = ota_params_read(&params);
    if (err == OTA_PARAMS_FLASH_ERROR || err == OTA_PARAMS_ERROR || err == OTA_PARAMS_CHECKSUM_ERROR) {
        LOGW("ota_confirm_boot_success: failed to read params, err=%d", err);
        return OTA_ERR_FLASH_WRITE;
    }
    /* err == OTA_PARAMS_INVALID_MAGIC: 全新设备, 没有 PENDING/SUCCESS 状态需要确认
     *      直接返回 OK, 避免新设备上"确认启动"误报为错误 */

    /* 标记本次启动已确认 */
    g_boot_confirmed = 1;

    /* 获取当前运行分区 (使用代码地址判断, 不依赖 RAM 地址) */
    running_app = get_running_app();
    LOGI("ota_confirm_boot_success: detected running_app=%lu (active_app in params=%lu)",
         (unsigned long)running_app, (unsigned long)params.active_app);

    /* 根据 ota_flag 状态分别处理 */
    switch (params.ota_flag) {
        case OTA_FLAG_PENDING:
            /* 简化设计: 联网成功即视为升级成功
             * 主动写 SUCCESS 到 Flash, 立即通知服务器 */
            LOGI("ota_confirm_boot_success: PENDING -> SUCCESS (new fw verified, app=%lu, count was %lu/%lu)",
                 (unsigned long)running_app,
                 (unsigned long)params.boot_count, (unsigned long)params.max_boot_count);

            /* 升级成功, 锁定当前分区为有效, 旧分区变为无效 (可被下次 OTA 覆盖) */
            params.ota_flag = OTA_FLAG_SUCCESS;
            params.boot_count = 0;  /* 重置计数器 */

            if (running_app == APP_A) {
                params.app_a_valid = 1;
                params.app_b_valid = 0;
                /* 同步当前分区的版本号 (以防 params 与实际固件版本不一致) */
                /* 注: 这里不修改 app_a_version, 由 ota_trigger_upgrade 写入时设置 */
            } else {
                params.app_b_valid = 1;
                params.app_a_valid = 0;
            }

            /* 写入 Flash (注意: 这个写入是必要的, 否则下次启动 bootloader
             * 还会把 ota_flag 当 PENDING 处理, 触发不必要的安全网逻辑) */
            err = ota_params_write(&params);
            if (err != OTA_PARAMS_OK) {
                LOGE("ota_confirm_boot_success: failed to write SUCCESS, err=%d", err);
                /* 即使写失败, 也尝试通知服务器 (因为升级客观上是成功的) */
            } else {
                LOGI("ota_confirm_boot_success: SUCCESS written to Flash");
            }
        ...
        case OTA_FLAG_SUCCESS:
            /* 已是 SUCCESS (可能上次启动就标记了, 或 bootloader 兜底写了) */
            LOGI("ota_confirm_boot_success: already SUCCESS, will notify server");
            ota_notify_boot_success();
            break;

        case OTA_FLAG_NORMAL:
            /* 正常运行 (非升级后) */
            LOGI("ota_confirm_boot_success: NORMAL state, no action");
            break;

        case OTA_FLAG_ROLLBACK:
            /* 回滚状态: 通知服务器升级失败 */
            LOGW("ota_confirm_boot_success: ROLLBACK detected");
            ota_notify_boot_success();
            break;

        default:
            LOGW("ota_confirm_boot_success: unknown ota_flag=%lu", (unsigned long)params.ota_flag);
            break;
    }
...
/**
 * @brief 检查是否可以开始新的 OTA (即旧分区是否可"删除")
 *
 * 业务含义 (按用户的设计):
 *   "只有新固件已经联网成功, 旧固件分区才能被作为 target 覆盖"
 *
 * 实现:
 *   只有 OTA_FLAG_NORMAL (正常) 或 OTA_FLAG_SUCCESS (升级成功) 才能开新 OTA
 *   处于 PENDING (待验证) 时拒绝新 OTA, 避免破坏正在验证的新固件
 *
 * @return 1 可以开始新 OTA, 0 拒绝
 */
int ota_is_ready_for_new_upgrade(void)
{
    ...
    if (err == OTA_PARAMS_FLASH_ERROR || err == OTA_PARAMS_ERROR || err == OTA_PARAMS_CHECKSUM_ERROR) {
        /* 真正的硬件/数据错误, 拒绝新 OTA */
        LOGW("ota_is_ready_for_new_upgrade: params read error, err=%d", err);
        return 0;
    }
    /* 注意: err == OTA_PARAMS_INVALID_MAGIC 是"出厂全新设备"的正常情况
     *      此时 ota_params_read() 已经用默认值填充了 params (ota_flag = NORMAL)
     *      不应该拒绝, 应该继续判断 flag */

    if (params.ota_flag == OTA_FLAG_PENDING) {
        LOGW("ota_is_ready_for_new_upgrade: previous upgrade still PENDING, refuse new OTA");
        return 0;
    }

    LOGI("ota_is_ready_for_new_upgrade: OK (err=%d, flag=%lu)", err, (unsigned long)params.ota_flag);
    return 1;
}
...
void ota_parse_command(const char *payload)
{
    ...
    if (strcmp(cmd, "ota_start") == 0) {
        // 0. 安全检查: 之前的升级是否还在 PENDING 验证中
        //    如果新固件还没确认"能联网", 拒绝擦除旧分区 (避免覆盖可能需要回滚的旧固件)
        if (!ota_is_ready_for_new_upgrade()) {
            LOGE("OTA: refused - previous upgrade is still pending verification");
            g_ota_status.error_code = OTA_ERR_INVALID_PARAM;
            g_ota_status.stage = OTA_STAGE_FAILED;
            ota_report_status();
            return;
